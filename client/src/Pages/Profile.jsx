...
const handleImageChange = async (e) => {
  ...
  try {
    ...
    // Send to backend - wrapped in try/catch specifically for this call
    try {
...
// const initialData = {
//   profile: { name: "", email: "", phone: "", location: "", current: "", new: "", confirm: "" },
//   // ── NOTIFICATIONS & PREFERENCES commented out (no backend endpoint yet) ──
//   // notifications: {
//   //   deviceAlerts: true,
//   //   securityAlerts: true,
//   //   energyReports: false,
...
//         }
//       } catch (err) {
//         // Resident or admin with no home yet — not a critical error
//         console.warn("[Profile] home fetch:", err.message);
//       } finally {
//         setHomeLoading(false);
//       }
...
//   // ── Unified save handler — branches per active tab ───────────────────────
//   const handleSave = async () => {
...
//     // ── Notifications / Preferences — local only (no backend endpoint yet) ──
//     // TODO: once backend adds PATCH /users/me/preferences and
//     //       PATCH /users/me/notifications, wire them up here.
//     showToastMsg("Settings saved successfully!");
//   };
...
//   // Tabs — "home" added; notifications & preferences kept as-is
//   const tabs = ["profile", "home", "notifications", "preferences", "security"];
...
//                     placeholder={value}
//                     value={data.profile[key]}
//                     onChange={e => manager.updateField("profile", key, e.target.value)}
//                     readOnly={key === "email"}
//                     style={{ padding: "10px", borderRadius: "8px", border: `1px solid ${colors.inputBorder}`, backgroundColor: key === "email" ? "#e8e8e8" : colors.inputBg, fontSize: "14px", cursor: key === "email" ? "not-allowed" : "text" }}
//                   />
//                 </div>
//               ))}
//               <button onClick={handleSave} style={buttonStyle(primaryColor)}>Save Changes</button>
...
//           {/* ── Home tab (new — connected to GET/PATCH /homes) ── */}
//           {activeTab === "home" && (
//             <>
//               {homeLoading ? (
//                 <p style={{ color: colors.gray, fontSize: "14px" }}>Loading home info...</p>
...
//                   {/* Editable fields — only admin can save changes */}
//                   <div style={{ display: "flex", flexDirection: "column", marginBottom: "20px" }}>
//                     <label style={{ fontSize: "12px", color: colors.gray, marginBottom: "6px" }}>Home Name</label>
//                     <input
//                       placeholder="Home name"
...
//           {/* ── Notifications tab (local only — no backend endpoint yet) ── */}
//           {/* TODO: wire to PATCH /users/me/notifications once backend adds it */}
//           {activeTab === "notifications" && (
//             <>
//               {Object.keys(data.notifications).map(key => (
//                 <div key={key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
...
//           {/* ── Preferences tab (local only — no backend endpoint yet) ── */}
//           {/* TODO: wire to PATCH /users/me/preferences once backend adds it */}
//           {activeTab === "preferences" && (
//             <>
//               <div style={{ display: "flex", gap: "10px", marginBottom: "15px" }}>
//                 {["fahrenheit", "celsius"].map(u => (
...
//         {/* Toast — shows real API error messages in red */}
//         {showToast && (
//           <div style={{ position: "fixed", bottom: "20px", right: "20px", backgroundColor: toastError ? "#c03030" : primaryColor, color: "#fff", padding: "12px 20px", borderRadius: "8px", fontSize: "14px", zIndex: 1000 }}>
//             {toastMessage}
//           </div>
